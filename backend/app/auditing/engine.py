...
class AuditEngine:
    ...
    def run(self, profile, run_id=None):
        ...
        try:
            enabled_platforms = [p for p in PLATFORMS if profile.platforms.get(p)]
            disabled_platforms = [p for p in PLATFORMS if not profile.platforms.get(p)]

            for platform in disabled_platforms:
                report["platforms"][platform] = {
                    "status": "disabled", "metrics": counters(), "queries": [], "sources": {},
                    "errors": [], "notes": [], "duration_seconds": 0
                }

            if enabled_platforms:
                max_workers = min(len(enabled_platforms), 8)
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = [executor.submit(self._audit_single_platform, p, profile, window, report, report_lock)

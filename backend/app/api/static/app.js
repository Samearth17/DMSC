...
async function renderTranscriptionSettings() {
  ...
  if (badge && banner) {
    if (s.indic_conformer?.available) {
      const dev = (s.indic_conformer.device || 'cpu').toUpperCase();
      badge.textContent = `AI4Bharat IndicConformer 600M: Ready on ${dev} · 22 Indian Languages (MIT License)`;
      banner.style.background = '#f0fdf4';
      banner.style.borderColor = '#bbf7d0';
      banner.style.color = '#166534';
    } else if (s.whisper?.installed) {
      badge.textContent = `OpenAI Whisper Engine: Installed · ` + (s.ffmpeg_installed ? 'ffmpeg Ready' : '(ffmpeg needed for local audio)');
      ...
      badge.textContent = `Using Cloud API or Subtitles`;
  ...
  if ($('transcribe-mode')) $('transcribe-mode').value = s.provider || 'indic_conformer';
  if ($('transcribe-language')) $('transcribe-language').value = s.selected_language || 'hi';
  if ($('transcribe-decoder')) $('transcribe-decoder').value = s.selected_decoder || 'rnnt';
  if ($('transcribe-model')) $('transcribe-model').value = s.model || 'base';
  if ($('transcribe-api-key') && s.whisper?.api_key_masked) $('transcribe-api-key').placeholder = `Configured (${s.whisper.api_key_masked}) — enter new key to replace`;
...
bind('save-transcription-btn', async () => {
  ...
  const language = $('transcribe-language')?.value || 'hi';
  const decoder = $('transcribe-decoder')?.value || 'rnnt';
  ...
  const payload = { provider, language, decoder, model, api_endpoint: endpoint, auto_transcribe: false };
...
async function runSingleVideoTranscription(saveToRecords) {
  ...
  const lang = $('test-transcribe-lang')?.value || $('transcribe-language')?.value || 'hi';
  const decoder = $('transcribe-decoder')?.value || 'rnnt';
  const resDiv = $('test-transcribe-result');
  resDiv.hidden = false;
  resDiv.replaceChildren(node('p', `Processing and transcribing audio using AI4Bharat IndicConformer [${lang}] (may take 10-30s)...`, 'muted'));
  try {
    const res = await api('/api/transcription/transcribe', { url, save: saveToRecords, language: lang, decoder });
    ...
    if (res.status === 'collected' && res.text) {
      ...
      const successMsg = saveToRecords 
        ? 'Transcribed & Saved to Watchtower Records (' + (res.provider || 'IndicConformer') + ')'
        : 'Transcription successful (' + (res.provider || 'IndicConformer') + ')';
